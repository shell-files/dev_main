...
const Onboarding = () => {
  ...
  const [metrics, setMetrics] = useState(() => {
    const saved = localStorage.getItem('onboarding_metrics_dummy');
    return saved ? JSON.parse(saved) : INITIAL_METRICS.map(m => ({ ...m }));
  });

  // 데이터 변경 시 로컬 스토리지에 자동 저장
  useEffect(() => {
    localStorage.setItem('onboarding_metrics_dummy', JSON.stringify(metrics));
  }, [metrics]);
  
  // 기존 DUMMY_USERS 대체: AuthContext의 실제 로그인 데이터 사용
  // [테스트용] 아래 변수 값을 바꾸면 다른 권한으로 화면을 확인할 수 있습니다.
  // "ESG담당자", "컨설턴트", "부서 담당자" 중 선택
  ...
  const handleSubmit = async (id) => {
    ...
    if (r.status) {
      ...
      const isDeptManager = role === "부서 담당자";
      const isConsultant = role === "컨설턴트";
      
      const colorId = m.category === 'E' ? 'E' : m.category === 'S' ? 'S' : m.category === 'G' ? 'G' : 'default';

      if (isDeptManager) {
        // 부서 담당자가 제출 시 -> ESG 담당자와 컨설턴트에게 알림
        addNotification({
          content: `${user?.name || '부서 담당자'}님이 데이터 승인을 요청하셨습니다.`,
          type: 'CHECK',
          title: '데이터 승인 요청',
          meta: { text: m.issueId, colorId }
        });
      } else if (isConsultant) {
        // 컨설턴트가 제출 시 -> ESG 담당자에게 알림
        addNotification({
          content: `${user?.name || '컨설턴트'}님이 데이터 승인을 요청하셨습니다.`,
          type: 'CHECK',
          title: '데이터 승인 요청',
          meta: { text: m.issueId, colorId }
        });
  ...
  const handleApprove = async (id) => {
    ...
    if (r.status) {
      ...
      // 알림 로직: 부서 담당자에게 승인 완료 알림
      const m = metrics.find(x => x.issueId === id);
      const colorId = m.category === 'E' ? 'E' : m.category === 'S' ? 'S' : m.category === 'G' ? 'G' : 'default';
      addNotification({
        content: `${id}번 승인이 완료되었습니다.`,
        type: 'CHECK',
        title: '데이터 승인 완료',
        meta: { text: id, colorId }
      });
    }
  };
  const handleReject = async (id) => {
    ...
    if (r.status) {
      ...
      // 알림 로직: 부서 담당자에게 반려 알림
      const m = metrics.find(x => x.issueId === id);
      const colorId = m.category === 'E' ? 'E' : m.category === 'S' ? 'S' : m.category === 'G' ? 'G' : 'default';
      addNotification({
        content: `${id}번 반려되었습니다.`,
        type: 'CHECK',
        title: '데이터 반려 안내',
        meta: { text: id, colorId }
      });
  ...
  const handleInvite = async () => {
    ...
    if (r.status) {
      ...
      addNotification({
        content: `${names} 담당자님께 초대 메일을 송부하였습니다.`, 
        type: 'USER', 
        title: '담당자 초대 발송', 
        meta: { text: modalIG, colorId }
      });
    }
  };
